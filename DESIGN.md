...
components:
  ...
  error-state:
    ...
    rounded: "{rounded.md}"
    padding: 24px
    iconColor: "{colors.signature-coral}"
  error-state-title:
    textColor: "{colors.ink}"
    typography: "{typography.title-sm}"
  error-state-retry:
  ...
  text-input-error:
    ...
    padding: 12px 16px
    height: 44px
    borderColor: "{colors.signature-coral}"
    borderWidth: 1.5px
  text-input-success:
    backgroundColor: "{colors.canvas}"
    textColor: "{colors.ink}"
...
- **Font licensing**: This spec uses Haas Grotesk / Haas Groot Disp, which is a licensed commercial font (Monotype/Linotype). Mobile app embedding requires a specific app-embedding license. If unlicensed, the fallback stack (`-apple-system, BlinkMacSystemFont, ...`) will be used. Consider using Inter Display (open-source, variable, already referenced in the pricing sub-system) as the universal typeface to avoid licensing risk.
- **Airtable marketing origin**: This design system was extracted from Airtable's marketing site (database SaaS). Some patterns — pricing sub-system (Inter Display, pill buttons, tier cards), marketing hero bands, full-bleed signature cards, partner logo strips — are specific to marketing pages and may not apply to the mobile document editor's UI (editor surface, chat interface, git UI, file browser). Evaluate each pattern against the app's actual screens before adopting.
